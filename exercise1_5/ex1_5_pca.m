...
figure(1)
plot(X(:,1), X(:,2), 'bo', 'MarkerSize', 7);
xlabel('x_1'); % label x-axis
ylabel('x_2'); % label y-axis
title('Initial 2D Dataset'); % add title
axis([0.5 6.5 2 8]); % set axis limits
axis square; % set aspect ratio to 1:1
...
figure(2)
plot(X_norm(:,1), X_norm(:,2), 'bo', 'MarkerSize', 7);
xlabel('x_1'); % label x-axis
ylabel('x_2'); % label y-axis
title('Normalized Dataset from myPCA'); % add title
...
axis square;
fprintf('\n\nProgram paused. Press enter to terminate.\n');
pause;
